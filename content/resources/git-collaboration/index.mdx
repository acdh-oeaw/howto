...
<Quiz>
  <QuizMultipleChoice question="What are GitHub, GitLab and Bitbucket?" answers={[{"answer":"Version control systems","kind":"incorrect"},{"answer":"Hosting providers","kind":"correct"},{"answer":"Code editors","kind":"incorrect"}]} messages={{"correct":"Correct!","incorrect":"Try again"}} />

  <QuizMultipleChoice question="What is the difference between `git fetch` and `git pull`?" answers={[{"answer":"They are the same.","kind":"incorrect"},{"answer":"`git fetch` allows to integrate all the changes that have been made to the remote repository into your local repo at the same time.","kind":"incorrect"},{"answer":"`git pull` allows to integrate all the changes that have been made to the remote repository into your local repo at the same time.","kind":"correct"}]} messages={{"correct":"Correct!","incorrect":"Try again"}} />
</Quiz>
