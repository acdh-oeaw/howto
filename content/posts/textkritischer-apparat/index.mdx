...
Den Zeilen sind die Lesarten im Apparat gegenüber gestellt. Unter dem Element `<app>`, findet sich der Eintrag im kritischen Apparat, in den das [Lemma](https://tei-c.org/release/doc/tei-p5-doc/en/html/ref-lem.html) `<lem>` bzw. die Lemmata und verschiedene Lesarten `<rdg>` der Textzeugen, auf die mit dem Attribut `wit` und der zugeordneten Identifikation hingewiesen wird, eingebettet sind. In unserem Beispiel wird ersichtlich, dass der Textzeuge _C_ an drei Stellen ein andere Lesart aufweist. Die Information ist in anderer Darstellung auch im physischen Apparat vorhanden.
