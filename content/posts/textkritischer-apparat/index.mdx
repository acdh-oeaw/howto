...
![Apparat](images/textkrit_apparat.png)
...
Bei dieser Variante sind Text (im `<body>`) und Apparat (im `<back>`) voneinander getrennt, aber im selben Dokument. Diese Variante wird mit dem Attribut `method="location-referenced" location="external"/\` im Element `<variantEncoding>`, das in das Element `<encodingDesc>` eingebettet ist,  im Header ausgewiesen. Um im Apparat verweisen zu können, muss die Textstelle im Text klar ausgewiesen sein. Die Referenz im Apparat erfolgt durch `<app loc=>`, und die entsprechende Identifikation. Im Beispiel weist die Identifikation den Seitenanfang der Seite 7 `<pb n=7>` und die erste Zeile `<l n=1>` aus.
