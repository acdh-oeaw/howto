...
Follow the steps that are explained in getting an access token, so that you can link your editor to
your Git account.

#### Cloning a repository and getting data

For example we want to clone `wboe-artikel` from https://gitlab.oeaw.ac.at/acdh-ch/wboe. The data at
this location, in this repository, is marked as private. So you need to log into gitlab.oeaw.ac.at
with the account you set up to see the contents.

On the top right there is a `Clone` button which, when clicked, gives you two options
`Clone with SSH` and `Clone with HTTPS`. We need the HTTPS option. On the right there is a copy to
clipboard button we can use.

If you installed Visual Studio Code there is a special kind of link that will open Visual Studio
Code if you allow it and start the cloning process with the right settings.
...
As you have already learnt in previous resources, git is very helpful in tracking changes in a
project.
...
Before uploading any changes, you want to track them locally. After cloning the wboe-repo, let's
open the `README.md` in Visual Studio Code. This is done while the uppermost mode on the left is
selected (the two sheets, files)
...
This only lists those files that we change since we last made our changes permanent, committed (to)
them. The README.md is listed. If we double click it we get the following view:
...
if the last commit was a while back and we don't remember exactly what happened.
...
We can add multiple files by selecting the names as usual using shift or ctrl or command and than
hit the `+`.
...
changes are added to create semantically meaningful units of changes. But the changing a staged
document will **not** propagate these changes in the next step. Keep that in mind.

When all related changes have been added to the "staging area", we can save them together as a
version history snapshot, a **commit**, with a **commit message** that briefly describes _why_
...
Code, you do not need to type the command but use the commit button.
...
right. Alternatively you can select "Terminal" and "New Terminal" from the menu bar
...
There is no graphical interface for this most often one time configuration. You need to copy the two
suggested commands and execute them in a terminal. Visual Studio Code provides one. So click on
...
Now replace `you@example.com` and `Your Name` with your name and execute two commands to configure
Git. You can copy the commands from above.
...
To get our changes to the server so other people in our team can fetch them to review them for
example we now `push` what we `committed` on our machine to the server.
...
1. If the two edit different lines in the file then the changes can be `merged` automatically That
   is a good solution in the majority of cases and it saves a lot of time.
...
_Note_: **Never** commit before resolving all conflicts, removing all conflict markers. You will
make the maker lines permanent and They are made like this to break any programming language and
XML.
