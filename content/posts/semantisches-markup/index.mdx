...
In diesem Beispiel finden Sie nochmals das Attribut `<type>`, diesmal um auf die Zeitschrift \_Die Filmwelt\_ zu verweisen. Ebenso werden zukünftige Zeitschriftenausgaben ausgezeichnet.
...
Diese Elemente, Attribute und Werte kommen in der folgenden praktischen Anwendung vor:
...
Im folgenden Beispiel sehen Sie einen Nachspann mit kurzen einer Liste historischer Personen. Neben dem Namen wird auch die ID `<xml:id="pe1">`, das Geschlecht, der Beruf, als auch  Geburts- und Sterbeort mit Datum ausgezeichnet. Beachten Sie bitte, dass diese ID mit einem Buchstaben oder Unterstrich beginnt und keine Sonderzeichen enthält (siehe \[hier](https://teibyexample.org/tutorials/TBED01v00.htm?target=globalattributes#xmlid) und im Kapitel *Einführung in die Makrostruktur*).
...
<Quiz>
  <Quiz.Card validateButtonLabel="Überprüfen">
    ...
    >
      <Quiz.Question>
        Als dritte Person soll nur Claude Farrère ausgezeichnet werden, dessen Roman Vorlag für dem Film _Die Schlacht_ war. Bitte nutzen Sie die nachstehenden Informationen, um eine richtige Auszeichnung für die Personenliste im Nachspann zu schreiben.
