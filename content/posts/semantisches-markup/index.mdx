...
* **Textabschnitte im `body`** `<div>` @type poem + @ana #verse: Der `body` kann durch das allgemeine Element `div` weiter in Abschnitte untergliedert werden, zudem in Vorspann `front`, Textkörper, oder Nachspann `back`. Das Attribut `@type poem` bestimmt ein Gedicht, `@ana #verse` steht für den Verweis auf einen analytischen Kommentar - das Gedicht ist in Versen geschrieben.
