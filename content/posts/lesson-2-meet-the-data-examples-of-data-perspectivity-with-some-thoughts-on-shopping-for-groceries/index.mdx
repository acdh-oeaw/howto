...
abstract: >-
  Working with seemingly similar data might be more complex than it seems. 
...
- Two TEI-XML files containing bibliographical data of the texts mentioned or quoted in _Kraus’ legal papers_.
...
But before we can seriously start to model our data, we first need to understand it because the scope and function of any given data model is not self-evident. Even less so is the meaning of any given data field in its respective data set. Every data set is the result of a modeling process, which, as Elena Pierazzo has pointed out, implies
