...
- **Personennamen** `<persName>`: Der Personenname bezeichnet den gesamten Namen einer Person, sowohl Vor- als auch Nachname. Wenn Sie Vorname `<forename>` und Nachname `<surname>`auch auszeichenn wollen, so würden Sie diese innerhalb des `<persName>`-Elements setzen.
...
- **Adressen** mit detailierter Auszeichnung: Sie können dem Element `<address>`auch die Auszeichnungen für Bezirke `<district>` und Straßen `<street>` unterordnen.

- **Ortsangaben** `<placeName>`: Mit diesem Element können Sie Orte auszeichnen.

- **Verweis** auf 'etwas' oder 'jemanden': Um auf bestimmte Personen, Dinge, Orte etc. zu verweisen, können Sie den 'referencing string' `<rs>` nutzen. Dies ist besonders hilfreich, wenn immer wieder dieselben Elemente in Ihren Text vorkommen.

- **Verweis** auf eine andere 'Quelle': Um auf andere Quellen innerhalb oder außerhalb Ihres Dokumentes zu verweisen, können Sie 'reference' `<ref>` nutzen. Zusätzlich können eine Kommentar oder Text ergänzen, wenn dies notwendig ist.

- **Textabschnitte im `body`** `<div>` @type poem + @ana #verse: Der `body`kann durch das Element `div`weiter untergliedert werden, in Vorspann `front`, Textkörper, oder Nachspann `back`. `@type poem`ist ein Attribut um ein zu bestimmen, `@ana verse` steht für den Verweis auf einen analytischen Kommentar - das Gedicht ist in Versen geschrieben.
...
Sehen Sie sich bitte das folgende Beispiel an, das sich dem Inhalt des Textes zuwendet. Identifiziert werden verschiedene Werktitel, Firmennamen, Adressen und Verweise. Darüber hinaus kommt das Attribut `@ana` ('Analyseverweis') zum Einsatz, um das Gedicht mit einem analytischen Kommentar zu verknüpfen. Um z.B. anzugeben, dass sich das Gedicht von Prosatexten unterscheidet, ist nach dem `<body>` ein Nachspann (`<back>`) zu ergänzen, der die Definition festhält.

Beispiele für `<title>` aus der Textvorlage:
...
`<rs>` kann z.B. auch dafür verwendet werden, Anaphern zu kodieren:

```xml
<p>'Marquis Yorisaka weilt in geheimer diplomatischer Mission in Paris; als <rs
  >er</rs> nach
Japan zurückkehrt ...'</p>
```
...
7. Zeichnen Sie den Ausdruck "erster" al Verweise auf eine Person aus mit `<rs>`.
8. Zeichnen Sie bitte innerhalb des zweiten Absatzes die zwei Spielstätten als Liste `<list>` aus, und die jeweilige Angabe zur Spielstätte als `<item>`.
...
Die Wunder des Meeres. Der Film „Die Wunder des Meeres“ führt uns in ein bisher nie gezeigtes Milieu: auf den Grund des Meeres. Der berühmte Ozeanforscher Sir J. E. Williamson hat als erster das nie vorher erhellte Dunkel der Meerestiefen mittels seiner eigens konstruierten Tiefseestation für kinematographische Aufnahmen dem menschlichen Auge enthüllt. Der Film »Die Wunder des Meeres« läuft Flotten-Kino, VI., Mariahilferstraße 85; Wienzeile-Kino, VI., Linke Wienzeile 4;
...
a) Ich verweise mit dem Attribut `@ana`. Damit verweise ich auf dem Kommentar.
...
d) Es genügt, wenn ich im `<back>`die Textart beschreibe und diese Beschreibung in den `<body>`kopiere.
...
Nachdem Sie nun schon die ersten Attribute und Werte kennengelernt haben, werden Sie nun Möglichkeiten einer genaueren Auszeichnung kennenlernen. Häufig wird Ihnen das Attribut `@type`begegnen, dem Sie dann einen bestimmten Wert zuweisen können, zum Beispiel `type=movie`, um zu zeigen, dass es sich um einen Film handelt. Auch das Attribut `@subtype`kann verwendet werden, um mehr in die Tiefe zu gehen. In vielen Fällen werden Sie Ihre Arbeit auf ein vorgefertigtes Schema aufbauen, das Sie Ihren Bedürfnissen anpassen können. Damit sind für Sie schon einige Attribute vorgegeben, die Sie nutzen können. Wenn Sie mehr über Schema erfahren möchten, lesen Sie bitte ![hier] (...) weiter. Bei der Auszeichnung sind Ihnen kaum Grenzen gesetzt; abhängig von Ihrer Forschungsfrage werden Sie sich aber für bestimmte Attribute entscheiden und festlegen, wie detailiert Sie Ihren Text auszeichnen.
...
![Filmwelt 31, 1924, S. 12, Filmrezension Die Schlacht](/assets/cms/images/posts/semantisches-markup/Die_Schlacht.jpg)
...
1. Claude Farrère ist die dritte _Person_, die ausgezeichnet werden soll. Nutzen Sie die ID-Nummer `"pe3"`. Er ist zudem Schriftsteller und männlich.
...
In der Rezension zum Film _"Die Schlacht"_ wird beschrieben, dass die Frau des Marquis Yorisaka sich in ein buddhistisches Kloster zurückzieht. Dem Text folgend, soll auch Buddha ausgezeichnet werden. Lesen Sie sich bitte die unten stehende Auszeichnung durch. Was fällt Ihnen im Vergleich zu vorangegangene Personenauszeichnungen auf? Wählen Sie bitte alle passenden Antworten aus.
...
_#==== Lösung - in finaler Version entfernen_ Alle vier Antworten sind richtig. _====#_
