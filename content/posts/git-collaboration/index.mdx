...
If we want to **clone** a local repository from a remote repository that we found on GitHub, we can copy it's URL and create a local copy on our machine in a directory of our choosing, using the command `git clone <url>`. 
HTTPS or SSH?
```bash
