...
\[Video 1: Introduction to the VOICE interface by Marie-Luise Pitzl, to follow soon. This clip offers a first introductory tour of the VOICE 3.0 Online interface and explains the main areas and buttons.]
...
\["VIDEO 2: VOICE 3.0 - Searches" - to follow soon]

### Bookmarks and Filters: Creating your own sub-corpus in VOICE 3.0

In VOICE 3.0 it is possible for users to create their own sub-corpora. In order to do so, you first of all navigate to the “Filter” tab in the left-hand area and turn on the filter options. You now have the possibility to select speech events according to their meta-data, for example, the number of speakers or interactants, power relations, duration, or L1 language.