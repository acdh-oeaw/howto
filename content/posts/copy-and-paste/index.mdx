...
In case anyone is asking themselves: Why create a project ontology at all? It’s a valid question, since re-use is the issue here and relying solely on pre-existing terms is the ideal to strive for. After all, if all classes entities have been assigned to and all properties linking these entities have their definitions elsewhere – in their original document – classes which have resolving URIs and linked to one another with properties to which this applies as well – what’s wrong with an RDF data set by itself, without an accompanying ontology, especially one that in essence would only combine pre-existing terms?
