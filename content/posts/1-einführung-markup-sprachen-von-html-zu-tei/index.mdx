---
title: 1. Einführung Markup-Sprachen - von HTML zu TEI
lang: de
date: 2021-07-23T10:36:13.905Z
version: 1.0.0
authors:
  - czeitschner-ulrike
  - schopper-daniel
editors:
  - königshofer-elisabeth
  - wiencek-florian
tags:
  - tei
  - xml
abstract: von XML zu TEI
licence: ccby-4.0
toc: false
uuid: ycJztJbuEaDJyNFBehyEe
---
* Digitalisat vs. born-digital
* Herkunft von Markup-Sprachen
* Einführung in Markup-Sprachen

  * Von HTML über XML bis TEI als Standard für Textannotationen
* Einführung von TEI (Über TEI) => Was ist TEI & was nicht?

## Einführung: XML / TEI Basics

* Elemente, Syntax, Dokumentmodell, Hierarchie
* Wohlgeformtheit, Validität
* Schema, Namensraum
* Was ist die TEI?

### Wie kommt mein Text in ein digitales Format?

Sofern es sich nicht um ein born digital-Dokument handelt - also ein Dokument das mit einem Computer erstellt wurde und deswegen bereits in digital verarbeitbarer Form vorliegt -, beginnt die
Erstellung einer digitalen Text-Ressource mit dem **[Einscannen einer Vorlage](https://dhmuseum.uni-trier.de/node/5)**. Dieser Scan resultiert in einem Pixelbild (z.B. im TIF, JPG oder PNG-Format) oder einem PDF. Das Aussehen der Print-Quelle ist in den Bilddaten ersichtlich, aber der textuelle Inhalt ist noch nicht digital weiter verarbeitbar. 

Deswegen erfolgt daran anschließend in einem zweiten Schritt die **Texterkennung mittels ["optical character recognition" (OCR)](https://de.wikipedia.org/wiki/Texterkennung)**. Das Ergebnis ist ein elektronischer Text z.B. in Form eines Word-Dokuments oder einer Text-Datei, den man zwar mit verschiedenen Programmen strukturieren und formatieren kann, wobei die Möglichkeiten sowohl im Hinblick auf das Layout als auch die inhaltliche Beschreibung meistens sehr begrenzt sind. Außerdem ist die Konvertierung in andere Formate oft nicht verlustfrei möglich, wenn die Formatierungs- oder Strukturinformationen in programmspezifischen Formaten abgespeichert sind. ([\> mehr Informationen zu Volltextdigitalisierung](https://dhmuseum.uni-trier.de/node/49))

Ziel der Erstellung elektronischer Ressourcen ist in unserem Fall die Nutzung in sprachwissenschaftlicher, literaturwissenschaftlicher oder kulturwissenschaftlicher Forschung. Um elektronische Daten systematisch, am besten mehrfach und interdisziplinär nutzen zu können, benötigen sie **dauerhafte** strukturelle und semantische und linguistische **Annotationen**. An diesem Punkt kommen sogenannte Auszeichnungs- oder Markupsprachen zum Einsatz. 

### Markupsprachen

Eine **Markupsprache** ist im Wesentlichen eine Strukturierungssprache, die es erlaubt, maschinenlesbar die Gliederung und Formatierung u.a. von Texten auszuzeichnen. Eine der bekanntesten "Markup Languages" (ML) ist die "[Hypertext Markup Language" (HTML)](https://www.w3.org/html/), die eine der technologischen Grundlagen des World Wide Webs bildet. Sie gliedert die multimodalen Inhalte einer Webseite so, dass ein Webbrowser aber auch Suchmaschinen und Algorithmen diese interpretieren und darstellen können. Hierfür kommt ein beschränktes Vokabular an vordefinierten "Tags" zum Einsatz, das in Web-Standards und Richtlinien des[ World Wide Web Consortiums (W3C)](https://www.w3.org/) festgelegt wurde und von standardkonformen Webbrowsern unterstützt wird. Im modernen Web-Publikationen werden Inhaltsstruktur bzw. strukturelle Semantik (z.B. Überschriftenebenen, Absätze, Listen, Hyperlinks, Zitate, und andere Medienelemente oder interaktive Elemente) und deren Darstellung getrennt. HTML übernimmt dabei die logische Strukturierung der Inhalte, während andere Technologien wie [Cascading Stylesheets (CSS)](https://www.w3.org/Style/CSS/Overview.en.html) oder [JavaScript](https://www.ecma-international.org/publications-and-standards/standards/ecma-262/) für die Darstellung und die Interaktion mit den Inhalten zum Einsatz kommen.

Für unsere Zwecke der wissenschaftlichen Nutzung der Daten bietet HTML mit seinem beschränkten und auf Grundbausteine für Web-Publikationen fokussierten Vokabular allerdings zu wenig Flexibilität in der Auszeichnung und damit Weiterverarbeitungsmöglichkeit. Dies würde letztlich auch die Forschungsfragen einschränken, die nach der Annotation an die Daten gestellt und durch die Daten beantwortet werden könnten. Deswegen bietet es sich an für die Modellierung und Strukturierung von Daten die erweiterbare Auszeichnungssprache ["Extensible Markup Language" (XML)](https://www.w3.org/XML/) zu verwenden.

XML ist ein einfaches, flexibles Format zur strukturierten Beschreibung von Daten. XML ist unabhängig von Hard- oder Software und besteht selbst aus Text, der sowohl für Menschen als auch für Maschinen lesbar und interpretierbar ist. Im Gegensatz zu HTML definiert XML selbst keine Auszeichnungselemente sondern stellt Regeln bereit, wie Elemente zu definieren und anzuwenden sind. Dadurch ist die Auszeichnungssprache beliebig erweiterbar.

### XML Syntax

Ein XML-Element besteht grundsätzlich aus einem **Start-Tag** und einem **End-Tag**, die den Inhalt umschließen. Tags werden von eckigen Klammern umschlossen. Der End-Tag wird immer mit einem "/" vor dem Elementnamen geschlossen. 

![XML-Element](images/xml_element.png "XML-Element, Bild: Ulrike Czeitschner, ACDH-CH")

Ein Beispiel für ein XML-Element ist z.B. folgendes in XML ausgezeichnete Datum:

```xml
<date>01.12.2018</date>
```

XML Elemente können auch **leer** bleiben. Hierfür gibt es 2 Arten, diese korrekt zu notieren:

```xml
a) <element></element>
b) <element/>
```

Mit **Attributen** und **Werten** lassen sich XML-Tags weiter ausdefinieren. Die Syntax dafür ist wie folgt:

```xml
<element attribut="wert">Inhalt</element>
<element attribut="wert" attribut2="wert"/>
```