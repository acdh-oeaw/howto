...
Ein TEI Dokument, in dem Sie ein Textdokument - sei es aus einer physischen Vorlage oder born digital - auszeichnen, besteht aus verschiedenen Abschnitten. Nach dem `<header>`, in dem die Metadaten ausgezeichnet werden, folgt die Auszeichnung Ihres Textes im Abschnitt `<text>`. In diesem Bereich können neben dem Fließtext auch ein Vorspann, Nachspannoder ähnliches ausgezeichnet werden. Der Fließtext befindet sich im sogenannten `<body>`-Element, wo Sie unter anderem die Struktur Ihres Fließtextes erfassen können. Diese Struktur folgt dabei zumeist dem Aufbau Ihres Textes. Haben Sie zum Beispiel einen Roman vor sich, werden Sie wahrscheinlich die Kapitel als aufeinander folgende Abschnitte auszeichnen. Erst durch Ihre Auszeichnung kann der Computer die Abschnitte, und genauso Überschriften, Strophen, Absätze, usw., als solche erkennen. Später können annotierte Elemente gefunden werden und aus Ihrer Struktur ergibt sich eine Hierarchie der Textteile, die für die Maschine lesbar ist. 
...
Das Glossar wurde [Einführung in die TEI, Christiane Fritze, IDE, 2019] (https://www.i-d-e.de/wp-content/uploads/2019/11/05_Einf%C3%BChrung-TEI.pdf) entnommen.
