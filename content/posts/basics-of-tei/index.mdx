...
Sie sehen eine Blanko-Vorlage und im zweiten Feld die ausgezeichnete Filmbeschreibung. Bitte beachten Sie, dass der `<header>` nicht vollständig ausgezeichnet wurde, da wir uns vor allem mit dme `<body>` beschäftigen.
...
Wie im vorangegangenen Beispiel sehen Sie, dass das Listenelement in ein `<div>`-Element eingebettet ist. Beachten Sie, dass die Listenelemente `<item>` wiederum in das Element `<list>` eingebettet sind, und so klar ersichtlich ist, dass alle Listenelemente teil der Liste sind. Achtung, die Listenelement sind nicht ineinander eingebettet, sondern stehen auf der selben Hierarchiestufe, also Geschwisterelemente sind.
