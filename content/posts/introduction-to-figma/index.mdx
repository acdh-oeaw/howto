...
Keep the "ImprintText" component selected. In the right-hand `Design` area, click on the `+` next to `Properties` and select `Variant`. This first variant will automatically be named "Default" - leave it like that. By clicking on the `+` (purple background) below the "ImprintText" component, you create a second variant. It is important that both variants are surrounded by the purple dashed line in order to be linked to the same component. In the `Design` area, you can now rename that second variant from "Variant2" to "Hover". (This is a recent change in the Figma interface: before, you had to click on `Variants` in the `Design` area and the component would automatically be duplicated.)
