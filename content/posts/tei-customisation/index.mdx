...
Unter einem Schema versteht man eine festgelegte Auswahl von TEI-Komponenten für den jeweiligen Text und Kontext, für den sie verwendet werden sollen. Einige dieser Schemata werden vom TEI Consortium zur Verfügung gestellt und setzen sich aus verschiedenen "Modulen" zusammen. Diese Module können genutzt werden, um bestimmte Textkomponenten auszuzeichnen. Werden sie kombiniert, so entsteht ein Schema, nach dem die Auszeichnung in TEI valid ist. Die Module enthalten verschiedenste Elemente und Attribute, die für den jeweiligen Zweck sinnvoll sind. Hier finden Sie eine vollständige Liste der [Module](https://www.tei-c.org/release/doc/tei-p5-doc/en/html/ST.html#STMA/); einige Beispiele sind:

- **header**: Die hier enthaltenen Elemente sind Ihnen teilweise schon aus dem Modul **TEI-Header** bekannt. Diese Elemente zeichnen vor allem Metadaten zu Ihrem Dokument aus.
- **namesdates**: Wie der Name schon sagt, enthält das Modul "Names, Dates, People, and Places" Elemente und Attribute um Informationen zu Personen, Zeitpunkten und Orten auszuzeichnen. Es beinhaltet beispielsweise das Element `<name>`, das durch die Attribute `<role="actor">` und `<type="person">` erweitert werden kann, um in einem Text eine\*n Schauspieler\*in auszuzeichnen.
...
- **Drama**: Dieses Schema eignet sich für die Auszeichnung dramatischer Texte, zum Beispiel Theaterstücke oder Filmskripte. Komponenten, die man hier findet, sind zum Beispiel `<castlist>`, um die Figuren auszuzeichen, Regieanweisungen, aber auch `<epilogue>`und `<prologue>` als spezifische Abschnitte des auszuzeichnenden Textes.
...
Folgenden Grundaktionen zur Anpassung stehen Ihnen zur Verfügung.
...
- neue Elemente (zu bestehenden Klassen) hinzufügen
  Achten Sie darauf, dass Ihr Schema auch nach den Anpassungen nach TEI_all gültig bleibt:

#### Praktische Anwendung

Sehen Sie sich im folgenden Beispiel die ersten Zeilen aus dem Schema an, das zur Validierung der Annotation unseres Beispieltextes **Die Filmwelt** benutzt wurde:
...
Neben der Angabe der Kodierung der genutzten Zeichen `UTF-8`, finden Sie hier auch die Angabe zum verwendeten Schema. Die Auszeichnung mit Referenz-URI, `?xml-model href="http://www.tei-c.org/release/xml/tei/custom/schema/relaxng/tei_all.rng"` zeigt, dass diese Auszeichnung in [`relaxng`](https://relaxng.org/compact-tutorial-20030326.html) validiert wird. **Relax NG** ist eine Schema-Sprache für XML und ein Standard für Datenvalidierung. Das Relax NG-Schema definiert das Modell für die Struktur und den Inhalt der XML-Dokumente.  Alle Elemente, Klassen und Attribute, die in diesem Schema verfügbar sind, werden als gültig akzeptiert. Entsprechende Referenzen finden Sie auf der TEI-Website, sofern Sie sich auf ein bereits vorhandenes Schema beziehen.
...
<Quiz>
  <Quiz.Card validateButtonLabel="Überprüfen">
    <Quiz.MultipleChoice>
      <Quiz.Question>
        Wie können Sie vorgehen? Wählen Sie alle richtigen Antworten aus.
      </Quiz.Question>

      <Quiz.MultipleChoice.Option>
        a) Ich nutze das Schema _'Speech'_, da der Text auch in gesprochener Sprache präsentiert werden könnte, und ich so phonetische Phänomene hervorheben kann.
      </Quiz.MultipleChoice.Option>

      <Quiz.MultipleChoice.Option isCorrect>
        b) Ich stelle sicher, dass das Modul _verse_ Teil des Schemas ist, das ich verwende.
      </Quiz.MultipleChoice.Option>

      <Quiz.MultipleChoice.Option>
        c) Ich arbeite mit dem Schema, das mir eine erfahrene Kollegin empfiehlt. Auf ihr Wissen kann ich vertrauen.
      </Quiz.MultipleChoice.Option>

      <Quiz.Message type="correct">
        Sehr gut! _TEI-Lite_ bietet genug Auswahl, um diesen kurzen Text auszuzeichnen. Wenn Sie das Modul _verse_ nutzen, können Sie die Struktur eines Gedichts auszeichnen.
      </Quiz.Message>

      <Quiz.Message type="incorrect">
        Leider falsch, überlegen Sie nochmal!
      </Quiz.Message>
    </Quiz.MultipleChoice>
  </Quiz.Card>
</Quiz>
...
<Quiz>
  <Quiz.Card>
    <Quiz.MultipleChoice>
      ...
      <Quiz.MultipleChoice.Option isCorrect>
        e) Elemente alphabetisch sortiert anzeigen
      </Quiz.MultipleChoice.Option>
...
In TEI finden sich über 500 Elemente. Wie Sie inzwischen wissen, finden sich diese Elemente (und ihre Attribute) in den TEI-Modulen. Doch auch wenn die Elemente einem Modul zugeordnet sind, haben viele Elemente und Attribute Gemeinsamkeiten, die über die Grenzen der Module hinausgehen. Zur besseren Übersicht und Verständlichkeit dieser Gemeinsamkeiten wurden die Elemente in Klassen eingeteilt. Bevor man sich näher mit den Klassen einzelner Elemente beschäftigt, muss angemerkt werden, dass zwei unterschiedliche Möglichkeiten des Ausdrucks von Gemeinsamkeiten gibt: wenn man Elemente danach einteilt, dass sie dieselben Attribute verwenden können, werden sie über **Attributsklassen** klassizifiert. Werden Elemente danach klassifiziert, dass sie im 'content model_'_ am selben Ort in der Texthierarchie vorkommen, spricht man von **Model-Klassen**.

<SideNote type="note" title="Ererbte Eigenschaften">
Die Mitglieder einer Klasse teilen bestimmte Eigenschaften miteinander. Diese Eigenschaften werden selten im Element selbst definiert, sondern in einem [classSpec](https://tei-c.org/release/doc/tei-p5-doc/de/html/TD.html#TDCLA) - Element, das Referenzinformation über die Elemente in derselben Attributs- oder Model-Klasse enthält. Man kann zu jedem Element oder Attribut, das man nutzt, in der Dokumentation nachlesen, welchen Klassen es angehört und welche Eigenschaften damit verbunden sind. Ebenso werden Eigenschaften "vererbt", wenn die Klasse eines Elements oder Attributs einer Über- oder Unterklasse zugeordnet ist. Wenn Sie also ein Element [mit einem bisher dort nicht genutzten Attribut verknüpfen](https://tei-c.org/release/doc/tei-p5-doc/de/html/USE.html#MDMDAL) wollen, so müssen Sie das Element der entsprechenden Attributsklasse hinzufügen.
</SideNote>
...
**Beispiel:** Die Attributklasse [`<att.canonical>`](https://www.tei-c.org/release/doc/tei-p5-doc/en/html/ref-att.canonical.html) umfasst die Attribute 'key' und 'ref', die autorisierte Informationen mit Elementen wie `<birth>`, `<country>`, `<title>`, oder auch Personennamen wie `<forename>` verknüpfen. Man muss also nicht bei jedem der genannten Elemente die Attribute definieren, sie werden durch die Zugehörigkeit zur Klasse zugewiesen. Gleichzeitig haben diese Elemente die genannten Attribute gemeinsam und sind durch sie miteinander verbunden. Jede Attributsklasse startet mit "att.".

Achtung: Die Attributsklasse `<att.global>` der globalen Attribute ist in jedem TEI-Modul definiert und deshalb immer verfügbar. Wenn Sie aber bestimmte Attributsklassen nutzen möchten, müssen Sie sicherstellen, dass diese in den von Ihnen für Ihr Schema gewählten Modulen verfügbar sind, da sie Sie ansonsten nicht nutzen können.
...
<Quiz>
  <Quiz.Card>
    <Quiz.MultipleChoice>
      <Quiz.Question>
        Im TEI-Dokument zu Die Filmwelt gibt es zwei Personenregister, in denen historische und fiktionale Personen getrennt aufgelistet werden. Wählen Sie bitte die zwei richtigen Attributsdefinitionen aus, die im ODD-Dokument aufscheinen sollen, damit Ihre Auszeichnung der Personenregister nachvollziehbar ist.
      </Quiz.Question>

      <Quiz.MultipleChoice.Option isCorrect>
        ```xml
            <valItem ident="historical">
             <desc>
      ...
      <Quiz.Message type="incorrect">
        Überlegen Sie noch einmal. Haben Sie zwei Optionen ausgewählt?
      </Quiz.Message>
    </Quiz.MultipleChoice>
  </Quiz.Card>
</Quiz>
...
Ein .odd-Dokument nutzt die "wenn-dann"-Logik einer Programmiersprache, indem bestimmte Regeln definiert werden, die sich als richtig oder falsch erweisen. Treten richtige Annahmen ein, folgen festgelegte Aktionen.

Im `<back>` zu _Die Filmwelt_ werden zwei Personenregister festgelegt; ein historisches und ein fiktionales. Damit die darin vorgenommenen Auszeichnungen oder fehlenden Auszeichnungen valide sind, müssen sie bestimmte Annahmen erfüllen:

> Wenn in einem Eintrag im Personenregister mehrere Namen angegeben sind, müssen diese jeweils explizit als Künstlername oder Geburtsname ausgewiesen sein. Ansonsten wird der Geburtsname angenommen.

Im Folgenden sehen Sie, wie diese Annahme im .odd-Dokument ausgezeichnet wird:

```xml
<rule context="tei:persName[count(parent::tei:person/tei:persName) gt 1]"
  xmlns="http://purl.oclc.org/dsdl/schematron">
    <assert test="@type=('artistName','birthName')">Mehrere Personennamen im Personenregister müssen durch die Typen "artistName" oder "birthName" unterschieden werden.
    </assert>
</rule>
<rule context="tei:persName[count(parent::tei:person/tei:persName)=1]"
  xmlns="http://purl.oclc.org/dsdl/schematron">
    <assert test="not(@type)">Hat eine Person im Personenregister nur einen Namen, darf dieser nicht typisiert sein (Geburtsname wird angenommen).
    </assert>
</rule>
```

Im ODD-Dokument wird nicht nur die formale Deklaration vorgenommen, sondern es wird auch durch für Menschen eindeutig lesbar beschrieben, was die gesetzte Regel bedeutet. Zusätzlich könnte auch ein Beispiel angegeben werden, um die Regel zu veranschaulichen.

### TEI Roma

Die Web-Applikation **[Roma](https://roma.tei-c.org/)** bietet ein Web-Interface, in dem ODDs, Schemata und Dokumentation generiert werden können. Sie wurde von Arno Mittelbach und Sebastian Rahtz geschrieben. Die Ausgabe kann in verschiedenen Formaten erfolgen, zum Beispiel DTD, W3C XML Schema, oder Relax NG XML, sodass verschiedene XML-Programme mit der Ausgabe arbeiten können.
...
Mittelbach, Arno, Rahtz, Sebastian. "Roma: generating customizations for the TEI," http://www.tei-c.org/Roma/.
