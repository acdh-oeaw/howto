...
tags:
  - prosopographic-data
  - semantic-web
abstract: "In the following HowTo, readers learn the basic terminology used in
  prosopographic data modelling and Semantic Web in relation to the projects
  NAMPI and VieCPro. The main concepts necessary to apply prosopographic data
  modelling are introduced: ontologies, vocabularies, RDF (Resource Description
  Framework), OWL (Web Ontology Language), SKOS (Simple Knowledge Organisation
  System), SPARQL (SPARQL Protocol and RDF Query Language), LOD (Linked Open
  Data) and LOD-cloud."
licence: ccby-4.0
toc: true
