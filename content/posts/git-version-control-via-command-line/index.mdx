...
Doing this will tell MacOS to get a view more programs from the internet. The bundle is called "Xcode Command Line Tools". So if this already happened for some reason you are ready. If not, you have to go through a few dialogs and after about 10 minutes everything is installed.
...
The `main` identifier is just a shortcut way to refer to the default timeline (it's actually the default _branch_ of the timeline, because there can be multiple parallel timelines 🤯. Branches were mentioned in the beginning and we'll have a look at these branches in the second post about [keeping repositories in sync: fetch, pull, push](/resource/posts/git-collaboration#keeping-repositorie-in-sync-fetch-pull-push).
