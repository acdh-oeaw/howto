...
"Xcode Command Line Tools". So if this already happened for some other reason you are ready.
If not you have to go through a few dialogs and after about 10 minutes everything is installed.
...
3. MacOS is likely to give you a warning, because it does not initially trust sourceforge.com as a source, as it is not a "verfied developer". 
...
If you are not afraid of the command line, you can use the package manager "Homebrew" to install
...
Note that other specialized editors also support managing git repositories themselves.
One notable example would be OxygenXML.
...
You also should make sure the `git-credential-libsecret` package is installed. This package exists on fedora based distributions.
On Ubuntu and debian you need to execute these commands in a terminal
```
