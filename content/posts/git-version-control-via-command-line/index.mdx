...
Many users, particularly Windows users, depend on using Graphical User Interfaces (GUI) in their daily lives and work schedules. GUIs have tabs, menus and windows that you can click on. GUIs are perceived as more user-friendly because they graphically represent the actions you take. 

The CLI relies on text input. The user needs to know syntax and commands to use it. When you open your CLI, it will look something like this:
```bash
C:\User\user_name>your_directory
```
Here's an example for a basic command to use in your CLI: to navigate to a directory, type **cd** and the directory name. To go back up, type **cd ..**.
```bash
C:\Users\user_name>cd your_directory
C:\Users\user_name\your_directory> cd a_small_folder
C:\Users\user_name\your_directory\a_small_folder> cd ..
C:\Users\user_name\your directory> cd ..
C:\Users\user_name>
```
</SideNote>
