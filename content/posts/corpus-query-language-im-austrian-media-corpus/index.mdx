...
`{0,1}` minimal 0 maximal 1 (entspricht ?), z.B.
...
- `<doc docsrc="STANDARD" & year="2017"/> Artikel aus der Zeitung „Der Standard“ aus dem Jahr 2017`
  (beachten Sie, dass ein **logisches &** verwendet werden muss)

Auch hier können in den Abfragen wiederum reguläre Ausdrücke verwendet werden, auf gleiche Weise wie es oben für den Inhalt von Token-Attributen demonstriert wurde:
...
In der Suchmaske steht neben dem Feld CQL die Angabe eines **Default attribute**. Bei Abfragen auf dieses Attribut kann die Schreibweise `[attribut="value"]` vereinfacht werden. Dabei ist **word** quasi der Default des Default. Es gilt also:

- statt `[ word="Paradeiser" ]` gilt vereinfacht: `"Paradeiser"` (die doppelten Anführungszeichen verpflichtend!)
...
- `[word="\.\.\."]` findet _punktipunktipunkti_ (3 Punkte)
...
<SideNote type="note">
  Ein Nebeneffekt dieser automatischen Aufteilung von `MULTIVALUE`-Attributen in ihre Einzelwerte ist allerdings, dass es bei Frequenzauswertungen zu Mehrfachzuordnungen kommt.
</SideNote>
...
![View Options Maske mit Auswahl Lemma und posTI](images/amc4.1_concordance_viewoptions_schützen_klima_kwic.png "Für View Options muss das Augen-Symbol ausgewählt werden. Die Funktionsmaske View Options mit Auswahl Lemma und posTI, und der entsprechenden Anzeige bei KWIC im Suchergebnis.")

### Frequency lists

_Nach_ einer erfolgten Suche können die Suchergebnisse unter dem Menüpunkt _Frequency List_ übersichtlich(er) zusammengefasst werden. Das ist oft auch hilfreich, um die Korrektheit einer Suchanfrage zu überprüfen.
