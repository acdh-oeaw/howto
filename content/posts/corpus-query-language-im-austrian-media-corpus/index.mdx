...
Diese vereinfachte Darstellung muss freilich um die Berücksichtigung von Satzzeichen ergänzt werden: Satzzeichen sollen vom vorangehenden Wort abgetrennt werden und bilden separate Token. Die Existenz von Abkürzungen verkompliziert die Tokenisierung: zum Beispiel sind die Punkte in _z.B._ natürlich keine Satzzeichen, dürfen daher nicht abgetrennt werden, denn _z.B._ sollte als ein einziges Token behandelt werden.

<SideNote type="warning" title="Fehleranfälligkeit: Sonderzeichen in Token">
Wie bei allen automatisierten Verarbeitungsschritten erfolgt die Tokenisierung bisweilen nicht fehlerfrei. So ist bei der Suche nach Token, die Satz- oder Sonderzeichen enthalten, zu berücksichtigen, dass evtl. die Tokenisierung diese Zeichen missinterpretiert hat und daher das Wort fälschlicherweise in mehrere Token aufgeteilt wurde.
</SideNote>
...
- `lc` : eine lower cased Version des Wortes. Die Suche in CQL ist _case sensitive_, `lc` kann verwendet werden, um case _insensitive_ -- also unter Großkleinschreibung ignorierend -- zu suchen. D.h. eine Suche `[word="Ciao"]` findet nur genau „Ciao“. Die Suche `[lc="ciao"]` findet ciao, Ciao, CIAO, CiAo etc.     
...
- `pos` : part-of-Speech tags, wie sie vom PoS tagger RFTagger vergeben wurden: zusätzlich zur Wortart werden auch morphologische Informationen angegeben.
  ...
  ![Einfache Suche im amc, Spalten mit Suche nach "ciao" als Lemma, Part of Speech type noun, im Bereich token](images/amc3.2_concordance_lemma_noun_token_ciao.png "Suche nach 'ciao' als Lemma")
...
Die Token sind typischerweise in **hierarchische Strukturen** eingebettet.
Zum besseren Verständnis dieses Konzepts hier ein Beispiel, das zeigt, wie Textdaten aussehen, die zum Export in die SkE vorbereitet wurden.  
...
<SideNote type="info" title="Info: Dokumentation ALLER Attribute im amc">
...
Im Gegensatz hierzu bietet die **CQL** weit mehr Möglichkeiten: Die Suche in beliebigen (Token-)Attributen, die Suche unter Berücksichtigung von Strukturen und deren Attribute, die Suche mit regulären Ausdrücken,
 etc.
...
Diese Bedingungen können auf mehrere Wörter ausgedehnt werden:
...
![Beispielhaftes Abfrageergebnis zur CQL-Abfrage Adjektiv + Haus. Links oben sehen Sie die Suchabfrage und die Anzahl der gefundenen Ergebnisse. Mittig finden Sie die ersten Suchergebnisse. Sie sehen in rot die gefundenen Formen im Text, in grau darunter das jeweilige Lemma.](images/amc_3.1_concord_cql_postt_adja_lemma_haus_research_result_dec22_boxes.png "Beispielhaftes Abfrageergebnis zur CQL-Abfrage Adjektiv + Haus. Links oben sehen Sie die Suchabfrage und die Anzahl der gefundenen Ergebnisse. Mittig finden Sie die ersten Suchergebnisse. Sie sehen in rot die gefundenen Formen im Text, in grau darunter das jeweilige Lemma.")
...
- „`_`“ : steht für 0, 1 oder beliebig viele Wiederholungen des vorhergehenden Zeichens oder Ausdrucks. z.B. `[word="Hallo_"]` findet „Hall“ „Hallo“ „Halloo“ etc.
...
- `([lemma="groß.*"] ([lemma="Haus"]|[lemma="Gebäude"])) | ([lemma="klein.*"] [lemma="(Hütte|Kate|Behausung)"])` sucht nach großen Häusern und Gebäuden oder kleine Hütten und Behausungen (aber findet auch "großindustrielle Häuser" und die "Kleinarler Hütte")
- `[lc="(h(a|e|i|o|u)){3,}"]` Eine alternative Formulierungsmöglichkeit um alle Wortformen zu finden, die aus mindestens 3 Wiederholungen von ha, he, hi ... bestehen

![Screenshot der Suche im AMC_4.1 des Ausdrucks "klima" mit mindestens zwei Wiederholungen, die als Nomen im Treetagger getaggt wurden.](images/amc_4.1_concordance_cql-word-klima.undposttnn.png "Screenshot der Suche im AMC_4.1 des Ausdrucks \\\"klima\\\" mit mindestens zwei Wiederholungen, die als Nomen im Treetagger getaggt wurden. Oben links sehen Sie den Suchausdruck, in der Mitte das Ergebnis. Alle gefundenen Begriffe sind als Nomen getaggt. Auch der Tippfehler  \\\\\\"klimafreundlichesMobilitätskonzept\\\\\\" und zusammengesetzte Nomen mit Bindestrich oder auch der Name der Webseite mit Punkt werden gefunden.")
...
- `[word="H[oa]se"]` findet also Hose und Hase - ist also äquivalent zu `[word="H(o|a)se"]`
- `[lc="[ptkbdg].+"] findet alle Wortformen, die mit einem Plosiv beginnen  - ist also äquivalent zu`\[lc="(p|t|k|b|d|g).+"]`
...
<Quiz>
  ...
  <Quiz.Card>
    <Quiz.MultipleChoice variant="multiple">
      <Quiz.Question>
        Mit \[] wird eine Auswahl an Zeichen definiert, sodass ein Suchergebnis oft der Verwendung von | gleich ist (zB \[word="h(o|a)se"] und \[word="h\[oa]se"] liefern dasselbe Ergebnis). Unten finden Sie ein Beispiel, bei dem dies nicht der Fall ist. Wählen Sie das Beispiel und die korrekte Begründung aus.
      </Quiz.Question>
    </Quiz.MultipleChoice>
  </Quiz.Card>
</Quiz>
...
In der ADVANCED Suchmaske befindet sich unter dem Feld CQL ein Auswahlfeld für ein **Default attribute**, das mit dem Wert _lemma_ vorausgefüllt ist. Bei Abfragen auf dieses Attribut ist **lemma** quasi der Default des Default. Es gilt also:
