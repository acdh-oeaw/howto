...
- „?“ : steht für 0 oder genau 1 Wiederholung des vorhergehenden Zeichens oder Ausdrucks, z.B. \[word="Hallo?"] findet genau „Hall“ und „Hallo“ etc. D.h. das ‚?‘ steht für Optionalität, aber nicht für Wiederholbarkeit. 
...
- `<S> Beginn-tag einer Struktur „S“`
- `</S> End-tag einer Struktur „S“ `
...
Strukturen haben bekanntlich möglicherweise ihre eigenen Attribute, und nach diesen kann wie folgt gesucht werden:
...
Für die Spezifikation der Werte der Strukturattribute können genauso reguläre Ausdrücke verwendet werden, wie es oben bereits für den Inhalt von Token-Attributen demonstriert wurde:
...
**containing**: wird verwendet um Einschränkungen auf Strukturen auszudrücken: es wird nach Strukturen gesucht, die bestimmte „Dinge“ enthalten. z.B. alle Sätze, aber nur, wenn sie das Wort „Haus“ beinhalten:
...
Typischerweise wird das eingesetzt um eine Kontexte zu filtern. 
...
<SideNote type="tip">
  Für solche kombinierten Filterungen sind in der NoSKE sogenannte Subkorpora oft hilfreich: Suchergebnisse lassen sich so einfach abspeichern, und nachfolgende Suchanfragen können dann auf das eben erstelle Subkorpus angewandt werden, und damit stufenweise verfeinert werden. 
So kann z.B. das Ergebnis der obigen Abfrage als "Ball"-Subkorpus gespeichert werden. In weiteren Abfragen könnte man z.B. Artikel aus diesem Subkorpus ausschließen in den sportspezifische Wörter wie "Tor", "Elfmeter", "Torhüter", "Stürmer" etc. vorkommen.  
</SideNote>
...
- statt `[ lemma="Paradeiser" ]` kann (bei einem _default attribute_ "lemma") vereinfacht  `"Paradeiser"` geschrieben werden (die doppelten Anführungszeichen bleiben aber verpflichtend!)
...
- `[posTT="ADJ.*"] [posTT="NN]` findet also alle Adjekiv-Nomen-Kombinationen finden, auch über Satz-, Paragraphen- und sogar über Dokumentgrenzen hinweg!
...
In der SKE können Attribute als `MULTIVALUE` definiert werden. Das bedeutet, solche Attribute enthalten potenziell mehrere unterschiedliche Werte, die durch ein Trennzeichen (`MULTISEP`) voneinander abgegrenzt werden. Das ist im amc bei vielen Attributen des `<doc>`-Elements der Fall. So könnte z.B. kann das Attribut **mutation** eine Wert `"Länder,Abend,Morgen"` aufweisen: es ist als `MULTIVALUE`-Attribut definiert, das Kommas (`,`) als `MULTISEP` verwendet.
...
<SideNote type="note">
  ...
  Eine Frequenzanalyse der Anzahl der `<doc>` pro mutation lieferte in unserem Beispiel, für ein _einziges_ `<doc mutation="Wien,Abend,Morgen" .../>` gibt, folgende 4 Ergebnisse. 
...
der Menüpunkt _Corpus info_ hilfreich. Hier werden nicht nur statistische Daten über das Korpus angezeigt, sondern auch die Namen der verfügbaren Token-Attribute, Strukturen und Struktur-Attribute.

Informationen zu den Strukuren finden sich dabei unter dem Titel "TEXT TYPES". Informationen zu den jeweiligen Struktur-Attributen werden durch einfaches Klicken auf einen Strukrturnamen ausgeklappt.   

### Anzeige anpassen

_Nach_ einer Suche kann über Auswahl der **View options** ( das Augen-Symbol in der Werkzeugleiste) die Anzeige angepasst werden. Hilfreich ist, dass hier die Anzeige einzelner Attribute oder Strukturen ein- oder ausgeblendet werden kann. So kann man z.B., um sich wieder einmal zu erinnern, wie die Werte des pos-Attributs bei Nomen aufgebaut sind, nach einem Nomen suchen, und dann in den View Options das Attribut pos auf _sichtbar_ schalten. So kann man sich ein Attribut anhand exemplarischer Beispiele erschließen.
...
![View Options Maske mit Auswahl Lemma und posTI](images/amc4.1_concordance_viewoptions_schützen_klima_kwic.png "Für View Options muss das Augen-Symbol ausgewählt werden. Die Funktionsmaske _View options_ mit Auswahl lemma und posTI, und der entsprechenden Anzeige bei KWIC im Suchergebnis.")

### Frequency

_Nach_ einer erfolgten Suche können die Suchergebnisse mit der Option  _Frequency_ aus der Werkzeugleiste übersichtlich(er) zusammengefasst werden. Das ist oft auch hilfreich, um die Korrektheit einer Suchanfrage zu überprüfen.
...
<SideNote type="note">
Die _Frequency_ Auswertung ist natürlich auch _das_ essentielle Werkzeug für alle quantitativen Untersuchungen. Typischerweise werden damit die Ergebnisse einer CQL-Abfrage nach Metadaten-Fakoren wie `doc.region` (Region) oder `doc.year` (Erscheinungsjahr) ausgezählt. Die Ergebnisse können dann in unterschiedlichen Formaten (`txt xls xml csv`)  heruntergeladen und für die   Weiterverarbeitung am lokalen Rechner gespeichert werden.
</SideNote>
...
Alternativ können einfache Einschränkungen auf Basis der Metadaten (also auf Basis der Strukturattribute des `<doc>`-Elements) auch im SKE _user-interface_ vorgenommen werden.
