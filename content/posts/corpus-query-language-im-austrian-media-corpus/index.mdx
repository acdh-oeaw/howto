...
Die einfachste Form der Suche im amc ist die sogenannte **Simple query**.
...
- entspricht die Sucheingabe auch einem Lemma, dann auch alle Flexionsformen dieses Lemmas. (D.h. eine Suche nach _Haus_ findet daher _auch_ alle Flexionsformen des _lemmas Haus_: das sind _Haus, Hauses_, _Hause_, _Häuser_, _Häusern_.)
...
In der Simple query ist die Verwendung des Wildcards  `*`  (Jokerzeichen, Asterisk) möglich: es steht für beliebig viele (auch null!) beliebige Zeichen.
...
Im Gegensatz hierzu bietet die **CQL** weit mehr Möglichkeiten: Die Suche in beliebigen (Token-)Attributen, die Suche unter Berücksichtigung von Strukturen und deren Attributen, die Suche mit regulären Ausdrücken,
 etc.
...
Suchanfragen mit der CQL werden wieder unter **Concordance**, diesmal im Bereich **Advanced** unter Auswahl des **Query type** **CQL** eingegeben. 
...
- `[ word="Haus" ]` Alle Einträge mit der genau der Buchstabenfolge „Haus“
- `[ lemma="Haus" ]` Alle Einträge mit dem Lemma „Haus“ – also „Haus“ „Hauses“ … „Häusern“
- `[ posTT="NN" ]` Alle Einträge mit dem PoS-tag "NN"   – also alle Nomen
- `[]` (KEINE Bedingungen)  Da hier für das Token keine weiteren Einschränkungen angegeben  wurden  findet diese Suche ausnahmlos ALLE Token

Für die Suche nach Mehrwortausdrücken können die Bedingungen für die Einzelworte einfach aneinandergereiht werden:
...
Außerhalb von Anführungszeichen werden Leerzeichen in CQL-Abfragen völlig ignoriert. Es macht also nur einen Unterschied in der _Lesbarkeit_ und nicht im Suchergebnis ob man `[  posTT = "ADJA"  ]  [  lemma = "Haus"  ]` oder `[posTT="ADJA"][lemma="Haus"]` schreibt.
</SideNote>
...
- „`?`“ : steht für 0 oder genau 1 Wiederholung des vorhergehenden Zeichens oder Ausdrucks, z.B. `[word="Hallo?"]` findet genau die beiden Formen „Hall“ und „Hallo“. D.h. das ‚`?`‘ steht für Optionalität aber nicht für die Wiederholbarkeit des linken Nachbarn von ‚`?`‘ . 
...
Beachten Sie den Unterschied zur Wirkungsweise des `*` in der Simple query: dort steht ein `*` allein schon als Platzhalter für 0, 1, oder beliebig viele beliebige Zeichen.
...
Die abstrakte Syntax für diesen Operator lautet:
...
- `[^abc]` ein "^" unmittelbar am Anfang der Zeichenklassendefinition bedeutet, dass die Zeichenklasse aus allen Zeichen _außer_ den angeführten besteht!
- [lc="[^ptkbdg].+"] findet alle Wortformen, die _nicht_ mit einem Plosiv beginnen. 
...
![Tabelle mit dem Suchergebnis im AMC 4.2., Ball in der Mitte hervorgehoben](images/amc_4.2._lemmaballnotinsports.png "Die oben genannte Suche liefert im AMC 4.2. Ergebnisse, die sich auf verschiedenste Tanzveranstaltungen, die hier \\\\\"Bälle\\\\\" genannt werden, beziehen. Die mit containing gesuchten Lemma müssen nicht in direkter Nachbarschaft des Suchbegriffs \\\\\"Ball\\\\\" vorkommen, sehr wohl aber in den Artikeln, aus denen die Suchergebnisse stammen.")
...
![View Options Maske mit Auswahl Lemma und posTI](images/amc4.1_concordance_viewoptions_schützen_klima_kwic.png "Für View Options muss das Auge-Symbol ausgewählt werden. Die Funktionsmaske _View options_ zeigt die Auswahl lemma und posTI. Darunter sieht man die entsprechende Anzeige bei KWIC im Suchergebnis.")
